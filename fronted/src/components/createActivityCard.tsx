...
const CreateActivityCard = () => {
  ...
  const {
    ...
    setTasksTimesAlreadyCatches: (value: any) => void; // Adjust 'any' to the appropriate type for setTasksTimesAlreadyCatches
    tasksTimes: any[]; // Adjust 'any' to the appropriate type for tasksTimes
  } = useAppContext();
  ...
  interface Headers {
    "Content-Type": string;
    "x-auth-token"?: string; // Make it optional if it might not always be present
  }
  ...
  try {
      const requestOptions: RequestOptions = user
          ? {
              method: "GET",
              headers: {
          ...
          : { method: "GET" }; // Provide a default RequestOptions when the user is not authenticated
      ...
      user
        ? setTasksTimesAlreadyCatches(
            data.tasks
              .filter(
                (activity: { activityTime: string; activityDate: string }) =>
                  activity.activityTime && activity.activityDate
              )
  ...
  const form = useFormik<ActivityFormValues>({
    initialValues: {
      activityName: "",
      activityDescription: "",
      activityAddress: "",
  ...
  function isTimeInArray(inputTime: string, inputDate: string, tasksTimes: []) {
